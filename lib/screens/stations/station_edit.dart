...
import 'package:plany_app/viewModal/controller.dart';
import 'package:plany_app/viewModal/register_view_model.dart';

class StationEdit extends StatefulWidget {
  StationEdit({Key key}) : super(key: key);
  @override
  _MyHomePageState createState() => _MyHomePageState();
}

class _MyHomePageState extends State<StationEdit> {
              ...
              TextButton(
                ...
                onPressed: () async {
                  if (_formKey.currentState.validate()) {
                    CreateStationRequestModel model =
                        CreateStationRequestModel();
                    model.name = _stationname.text;
                    model.color = _hexcolor.text;
                    await editContactViewModel.createStation(model);
                    if (editContactViewModel.createStationResponse.status ==
                        Status.COMPLETE) {
                      CreateStationResModel resModel =
                          editContactViewModel.createStationResponse.data;
                      ...
                      if (resModel.title == 'success') {
                        Fluttertoast.showToast(
                            ...
                            fontSize: 16.0);
                        _controller.isSetSelected(true);
